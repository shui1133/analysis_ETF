            ...
            if current_date.year != last_year or current_date >= end_date:
                if month_count > 0:
                    ...
                    for ticker in etfs:
                        # 獲取年末股價
                        pdf = etf_data[ticker]['price']
                        year_end_prices = pdf[pdf['日期'].dt.year == last_year]
                        if len(year_end_prices) > 0:
                            current_price = float(year_end_prices.iloc[-1]['收盤價'])
                        else:
                            current_price = 0.0
                        ...
                        etf_detail['etfs'][ticker] = {
                            ...
                            '市值': round(holdings[ticker] * current_price),
                            '權重': etfs[ticker]
                        }
